...
internal class TvShowLibraryTest {
    ...
    fun printShows() {
        ...
        val seriesWithColonEpisodeOne = Media("ttid06", "Series With: Colon: Episode Two", LocalDateTime.now(), TraktMediaType.EPISODE, 8, LocalDateTime.now(), 50)


        val outputStream = ByteArrayOutputStream()
        System.setOut(PrintStream(outputStream))
        TvShowLibrary(MediaList(listOf(seriesOneEpisodeOne, seriesTwoEpisodeOne, seriesWithColonEpisodeOne, seriesTwoEpisodeTwo, seriesOneEpisodeTwo))).printShows()
        assertEquals("Series One=TvShow(name=Series One, defaultWatchTime=00:00)" + System.getProperty("line.separator") +
                "Series Two=TvShow(name=Series Two, defaultWatchTime=00:00)" + System.getProperty("line.separator") + "Series With: Colon=TvShow(name=Series With: Colon, defaultWatchTime=00:00)" + System.getProperty("line.separator"), outputStream.toString())
